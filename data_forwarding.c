...
void handle_received_data(Packet packet, Byte packet_bytes[TCP_BUFFER_SIZE], NeighbourSwitch neighbour_switch)
{
	...
	// // 		fflush(stderr);
	// // 	}
	// // 	if (ip_address_equals(neighbour_switches[i].ip_address, packet.destination_ip))
	// // 	{
	// // 		fprintf(stderr, "aware of destination ip\n");
	// // 		fflush(stderr);
	// // 		// check if next hop is equal to the switch - SEND TO END POINT
	// // 		if (ip_address_equals(neighbour_switches[i].next_hop, packet.destination_ip))
	// // 		{
	// // 			fprintf(stderr, "next hop is equal to destination ip\n");
	// // 			fflush(stderr);
	// // 			// check if already ready and send
	// // 			time_t current_time = time(NULL);
	// // 			if (difftime(current_time, neighbour_switches[i].time_of_last_ready) < 5)
	// // 			{
	// // 				// ready
	...
	// // 			Byte *query_packet_bytes = packet_to_bytes(query_packet);
	// // 			if (send(neighbour_switches[i].socket_fd, query_packet_bytes, 12, 0) < 0)
	// // 			{
	// // 				perror("send failed");
	// // 			}
	// // 			// receive response packet
	// // 			Byte response_packet_bytes[TCP_BUFFER_SIZE];
	// // 			if (recv(neighbour_switches[i].socket_fd, response_packet_bytes, TCP_BUFFER_SIZE, 0) < 0)
	// // 			{
	// // 				perror("recv failed");
	// // 			}
	// // 			Packet response_packet = bytes_to_packet(response_packet_bytes);
	...
	// // 				return;
	// // 			}
	// // 			neighbour_switches[i].time_of_last_ready = time(NULL);
	// // 			// send data
	// // 			if (send(neighbour_switches[i].socket_fd, packet_bytes, TCP_BUFFER_SIZE, 0) < 0)
	// // 			{
	// // 				perror("send failed");
	...
	// // 		}
	// // 		// forward to next hop
	// // 		if (send(neighbour_switches[i].next_hop_socket_fd, packet_bytes, TCP_BUFFER_SIZE, 0) < 0)
	// // 		{
	// // 			perror("send failed");
	// // 		}
	// // 		return;
	...
	// {
	// 	// check if neighbour
	// 	if (neighbour_switches[i].socket_fd != neighbour_switches[i].next_hop_socket_fd)
	// 		continue;
